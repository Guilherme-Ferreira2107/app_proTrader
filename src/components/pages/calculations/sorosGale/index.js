...
const percent33 = 0.33;  
const percent67 = 0.67;
...
class SorosGale extends Component {
    ...
    orderA(){
        let temp = parseFloat(this.props.newOrder) * parseFloat(percent33);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderA2(){
        let temp = parseFloat(this.return()) * parseFloat(percent33);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderA3(){
        let temp = parseFloat(this.return2()) * parseFloat(percent33);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderA4(){
        let temp = parseFloat(this.return3()) * parseFloat(percent33);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderA5(){
        let temp = parseFloat(this.return4()) * parseFloat(percent33);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderB(){
        let temp = parseFloat(this.props.newOrder) * parseFloat(percent67);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderB2(){
        let temp = parseFloat(this.return()) * parseFloat(percent67);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderB3(){
        let temp = parseFloat(this.return2()) * parseFloat(percent67);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderB4(){
        let temp = parseFloat(this.return3()) * parseFloat(percent67);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    orderB5(){
        let temp = parseFloat(this.return4()) * parseFloat(percent67);
        var calc = parseFloat(temp.toFixed(2));
        return calc;
    }
    _Win = async () => {
        const { updateWallet, updateProfit, newProfit, newWallet } = this.props;
        let inputValue = (this.orderA())*(this.props.newPayout)/100;
        await updateWallet({ inputValue, newWallet });
        await updateProfit({ inputValue, newProfit });
    }
    _Loss = async () => {
        const { updateWallet, updateProfit, newProfit, newWallet } = this.props;
        let inputValue = -(this.props.newOrder);
        await updateWallet({ inputValue, newWallet });
        await updateProfit({ inputValue, newProfit });
    }
    _Win2 = async () => {
        const { updateWallet, updateProfit, newProfit, newWallet } = this.props;
        let inputValue = (this.orderA2())*(this.props.newPayout)/100;
        await updateWallet({ inputValue, newWallet });
        await updateProfit({ inputValue, newProfit });
    }
    _Loss2 = async () => {
    ...
    _Win3 = async () => {
        const { updateWallet, updateProfit, newProfit, newWallet } = this.props;
        let inputValue = (this.orderA3())*(this.props.newPayout)/100;
        await updateWallet({ inputValue, newWallet });
        await updateProfit({ inputValue, newProfit });
    }
    _Loss3 = async () => {
    ...
    _Win4 = async () => {
            const { updateWallet, updateProfit, newProfit, newWallet } = this.props;
            let inputValue = (this.orderA4())*(this.props.newPayout)/100;
            await updateWallet({ inputValue, newWallet });
            await updateProfit({ inputValue, newProfit });
        }
    _Loss4 = async () => {
    ...
    _Win5 = async () => {
            const { updateWallet, updateProfit, newProfit, newWallet } = this.props;
            let inputValue = (this.orderA5())*(this.props.newPayout)/100;
            await updateWallet({ inputValue, newWallet });
            await updateProfit({ inputValue, newProfit });
        }
    _Loss5 = async () => {
